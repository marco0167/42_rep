...
/*   Updated: 2023/03/23 13:16:02 by mcoppola         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	input_controll(int key, t_game *game)
{
	if (key == 53)
		ft_close_game(game, 4, "EXIT");
	if (key == 13 && (game->map.map
			[game->player_pos.y - 1][game->player_pos.x] != '1'))
		game->player_pos.y -= 1;
	else if (key == 1 && (game->map.map
			[game->player_pos.y + 1][game->player_pos.x] != '1'))
		game->player_pos.y += 1;
	else if (key == 0 && (game->map.map
			[game->player_pos.y][game->player_pos.x - 1] != '1'))
		game->player_pos.x -= 1;
	else if (key == 2 && (game->map.map
			[game->player_pos.y][game->player_pos.x + 1] != '1'))
		game->player_pos.x += 1;
}
